...
const ChatInterface: React.FC<ChatInterfaceProps> = ({ messages, input, setInput, onSendMessage, isLoading }) => {
  ...
  const renderContent = (content: string, role: 'user' | 'assistant') => {
    // Bersihkan teks daripada aksara \n literal jika model menghasilkannya sebagai teks
        ...
        {messages.map((m, idx) => (
          <div key={idx} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2`}>
            <div className={`flex gap-4 max-w-[95%] md:max-w-[85%] ${m.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
              <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 border ${m.role === 'user' ? 'bg-slate-700 border-white/10' : 'bg-indigo-600 border-indigo-400 shadow-[0_0_15px_rgba(79,70,229,0.3)]'}`}>
                {m.role === 'user' ? <User className="w-5 h-5" /> : <Bot className="w-5 h-5" />}
              </div>
              <div className={`relative px-5 py-4 rounded-[1.5rem] border ${m.role === 'user' ? 'bg-slate-700/80 border-white/10 rounded-tr-none shadow-lg' : 'bg-indigo-950/40 border-indigo-500/20 rounded-tl-none shadow-xl'}`}>
        ...
        {isLoading && (
          <div className="flex justify-start ml-14">
            <div className="bg-slate-800/60 p-4 rounded-2xl border border-white/10 flex items-center gap-3">
              ...
              <span className="text-xs text-slate-400">Memproses data...</span>
            </div>
          </div>
        )}
      </div>

      <div className="p-6 bg-gradient-to-t from-[#1e293b] via-[#1e293b] to-transparent">
        <div className="max-w-4xl mx-auto relative">
          <div className="flex items-center gap-2 bg-[#1a2333]/95 p-1.5 rounded-full border border-white/10 shadow-2xl focus-within:border-indigo-500/50 transition-all">
            <input 
              type="text"
              value={input}
            ...
            <button 
              ...
              className="w-12 h-12 rounded-full flex items-center justify-center bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-800 disabled:text-slate-600 transition-all shadow-lg active:scale-95"
            >
